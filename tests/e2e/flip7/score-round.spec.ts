...
import { createFlip7Game, fillRoundScores, scoreFlip7Round } from "./helpers";
...
test.describe("Flip 7 Score Management", () => {
  ...
  test("updates player scores and verifies the results", async ({ page }) => {
    await scoreFlip7Round(page, ["10", "25", "30"]);
  ...
  test("scores multiple rounds", async ({ page }) => {
    await scoreFlip7Round(page, ["10", "25", "30"]);
    await scoreFlip7Round(page, ["0", "32", "17"]);
  ...
  test("validates form", async ({ page }) => {
    await fillRoundScores(page, ["50", "string", "30"]); // middle value is invalid
  ...
  test("prevents adding scores to a completed game", async ({ page }) => {
    ...
    // score a winning round to complete the game (redirect is awaited inside)
    await scoreFlip7Round(page, ["201", "25", "30"]);
